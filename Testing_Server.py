...
class TestingServicer(ProtoFile1_pb2_grpc.TestingServicer):
    def TestService(self, request, context):
        comp, desig, sal = TestServiceMethods.provide_details(request.user_name, request.user_id)
        result = ProtoFile1_pb2.TestResp(company_name=comp, designation=desig, salary_eur=sal)
        return result

if __name__ == '__main__':
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=1))
    ProtoFile1_pb2_grpc.add_TestingServicer_to_server(TestingServicer(), server)
